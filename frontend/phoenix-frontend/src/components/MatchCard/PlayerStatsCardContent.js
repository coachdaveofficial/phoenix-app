...
export default function PlayerStatsCardContent({ isActive, loading, allTimeGoalScorers, allTimeAssisters, recentSeasonTopScorers, recentSeasonMostAssists, recentSeason }) {
                ...
                grid  xs:grid-cols-1  gap-2 p-4 content-start">
                    <div>
                        ...
                        <ul className="list-none pl-4">
                            {Array.isArray(allTimeGoalScorers) ?
                                allTimeGoalScorers.map((player, idx) => (
                                ...
                                <li className={listItemClass}>
                                    <div className="flex justify-between">
                                        <span>No goals have been recorded yet</span>
                                    </div>
                                </li>
                            }
                        </ul>
                    ...
                    <div>
                        <h2 className={titleClass}>{recentSeason.name} Most Goals</h2>
                        <ul className="list-none pl-4">
                    ...
                    <div>
                        ...
                        <ul className="list-none pl-4">
                            {Array.isArray(allTimeAssisters) ?
                                allTimeAssisters.map((player, idx) => (
                                    <li key={idx} className={listItemClass}>
                                        <div className="flex justify-between">
                                            <span>{player.player_name}</span>
                                ...
                                <li className={listItemClass}>
                                    <div className="flex justify-between">
                                        <span>No assists have been recorded yet</span>
                                    </div>
                                </li>
                            }
                        </ul>
                    ...
                    <div>
                        <h2 className={titleClass}>{recentSeason.name} Most Assists</h2>
                        <ul className="list-none pl-4">
                            {/* check to see if data is an array of players */}
                            {/* if data is not array, then there are no data for this season, display error message */}
